/**
 * Self-contained QR Code Generator (SVG & Canvas renderer)
 * Generates valid QR Codes (Type 1-10, ECC Levels L/M/Q/H)
...
(function (root, factory) {
  ...
  // Reed-Solomon & QR Table constants
  ...
  /**
   * Helper functions to generate SVG or Render to Canvas
   */
  var QRCode = {
    generateSVG: function (text, options) {
      options = options || {};
      var level = options.ecc || 'M';
      var eccLevel = QRErrorCorrectLevel[level] || QRErrorCorrectLevel.M;
      var qr = new QRCodeModel(0, eccLevel);
      qr.addData(text);
      qr.make();
      ...
      var fill = options.colorDark || '#00f2ff';
      var bg = options.colorLight || 'transparent';
      ...
      var svg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ' + totalCells + ' ' + totalCells + '" width="' + size + '" height="' + size + '" shape-rendering="crispEdges">';
      if (bg && bg !== 'transparent') {
        svg += '<rect width="100%" height="100%" fill="' + bg + '"/>';
      }
      svg += '<path d="' + paths.join(' ') + '" fill="' + fill + '" fill-rule="evenodd"/>';
      svg += '</svg>';
      return svg;
    },

    getMatrix: function (text, level) {
      var eccLevel = QRErrorCorrectLevel[level || 'M'] || QRErrorCorrectLevel.M;
      var qr = new QRCodeModel(0, eccLevel);
      qr.addData(text);
      qr.make();
      var count = qr.getModuleCount();
